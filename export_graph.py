...
python export_graph.py --checkpoint_dir checkpoints/20170424-1148 \
...
tf.flags.DEFINE_integer('image_length', '896', 'image length, default: 896')
tf.flags.DEFINE_integer('image_height', '48', 'image height, default: 48')
...
def export_graph(model_name, XtoY=True):
  ...
  with graph.as_default():
    cycle_gan = CycleGAN(ngf=FLAGS.ngf, norm=FLAGS.norm, image_length=FLAGS.image_length, image_height=FLAGS.image_height)

    input_image = tf.placeholder(tf.float32, shape=[FLAGS.image_length, FLAGS.image_height, 3], name='input_image')
    cycle_gan.model()
    if XtoY:
